# Hi there! My name is Timberfang 👋

Hello! You can call me Timberfang, and I've been learning to code on and off for the past couple years. While I'm not as dedicated as full-time software dev, I enjoy making tools and solving problems. I do a lot of automation & scripting in PowerShell, but more complex stuff is made in C#. I'm hardly an expert in either language, but I know enough to get by, at least. ☺️

- 📦 I self-host a private [Forgejo](https://forgejo.org) instance on my home network. Most of my repositories on GitHub are mirrored from there, including this one!
- 🕹️ Besides programming, some of my hobbies include writing, video games, and hiking!
- 🌎 I'm an environmentalist and an animal-lover, so taking care of our planet is important to me.
- ⚠️ When the moon's full and glowing bright, you might just find a werewolf behind the keyboard! Don't worry, I don't bite... much.

## Projects

I'll admit, most of my work is small hobby projects; I haven't made anything I would consider "complex" or "production-ready". Still, I had fun doing it, and I think that's worth something, too.

1. [CaesarCipher](https://github.com/Timberfang/CaesarCipher): My implementation of the classic [Ceasar cipher](https://en.wikipedia.org/wiki/Caesar_cipher) in C#. It's been done before by much better programmers than myself, but ever since I heard about it in college, I wanted to try my hand at it.
2. [TicTacToe](https://github.com/Timberfang/TicTacToe): The children's game tic-tac-toe / noughts and crosses / Xs and Os, right in your terminal.
3. [MediaTools](https://github.com/Timberfang/MediaTools): My first 'serious' project, this is a PowerShell module I'm working on to help me with preparing content for my personal Plex server. It provides a simple wrapper around the popular media processing suite [FFmpeg](https://ffmpeg.org) for common functions that I needed.
4. [LicenseIt](https://github.com/Timberfang/LicenseIt): A WIP command-line utility to generate `LICENSE` files for your projects.
5. [HelperScripts](https://github.com/Timberfang/HelperScripts): Snippets of code and scripts that I found useful at some point.

## Connect With Me

- [![Mastodon Badge](https://img.shields.io/badge/Mastodon-%40Timberfang-6364FF?style=flat&logo=mastodon&logoColor=lightblue)](https://pawb.fun/@Timberfang)
- [![Pixelfed Badge](https://img.shields.io/badge/Pixelfed-Timberfang-6366F1?style=flat&logo=pixelfed&logoColor=lightblue)](https://pixelfed.furryfandom.me/Timberfang)
- [![Lemmy Badge](https://img.shields.io/badge/Lemmy-u%2FTimberfang-000000?style=flat&logo=lemmy&logoColor=lightblue)](https://pawb.social/u/Timberfang)
- [![Fur Affinity Badge](https://img.shields.io/badge/Fur%20Affinity-Timberfang-36566F?style=flat&logo=furaffinity&logoColor=lightblue)](https://www.furaffinity.net/user/timberfang)
- [![Email Badge](https://img.shields.io/badge/Email-Timberfang.Code%40pm.me-6D4AFF?style=flat&logo=protonmail&logoColor=lightblue)](mailto:timberfang.code@pm.me)

## Skills

![C#](https://img.shields.io/badge/Programming-C%23-512BD4?style=flat&logo=csharp)
![PowerShell](https://img.shields.io/badge/Scripting-PowerShell-5391FE?style=flat&logo=powershell)
![.NET](https://img.shields.io/badge/Framework-.NET-512BD4?style=flat&logo=.net)
![Docker](https://img.shields.io/badge/Containers-Docker-2496ED?style=flat&logo=docker)
